...
import me.arasple.mc.litechat.formats.format.Format;
import me.arasple.mc.litechat.formats.format.PrivateFormat;
...
public class ChatFormats {
    ...
    public static void loadFormats(CommandSender... notify) {
        Bukkit.getScheduler().runTaskAsynchronously(LiteChat.getPlugin(), () -> {
            ...
            try {
                LiteChatFiles.getFormats().getMapList("NORMAL").forEach(x -> normal.add(new Format((Map<String, Object>) x)));
                LiteChatFiles.getFormats().getMapList("GLOBAL").forEach(x -> global.add(new Format((Map<String, Object>) x)));
                LiteChatFiles.getFormats().getMapList("STAFF").forEach(x -> staff.add(new Format((Map<String, Object>) x)));
                LiteChatFiles.getFormats().getMapList("PRIVATE-SENDER").forEach(x -> private_sender.add(new PrivateFormat((Map<String, Object>) x)));
                LiteChatFiles.getFormats().getMapList("PRIVATE-RECEIVER").forEach(x -> private_receiver.add(new PrivateFormat((Map<String, Object>) x)));
            } catch (Throwable e) {
                for (CommandSender sender : notify) {
                    sender.sendMessage("§8[§3Lite§bChat§8]§8[§6WARN§8] §6--------------------------------------------------");
                    sender.sendMessage("§8[§3Lite§bChat§8]§8[§7INFO§8] §6加载聊天格式过程中发生错误, 请检查配置节点. 抛出异常如下");
    ...
    private static void checkDepends() {
        ...
        List<String> dependExpansions = LiteChatFiles.getSettings().getStringList("GENERAL.DEPEND-EXPANSIONS");
